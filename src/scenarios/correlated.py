...
def generate_correlated_data(config):
    """
    試行ごとに相関があるシナリオのデータ生成を行う。
    - cvr_concentrationを用いてユーザー間の偏りをモデリング。
    - ユーザーを等分割（通常は50%/50%）。
    - repeat_multiplierを用いてリピート時のCVR変動をモデル化。
    """
    ...
    split_ratio = config.get("split_ratio", 0.5)  # 通常は0.5
